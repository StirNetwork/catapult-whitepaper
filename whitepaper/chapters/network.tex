	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\tikzset{
				...
				dropConnection/.style = { rounded, fill=gray!30, minimum width=1.5cm, text width=1.5cm },
				process/.style = { base, text width=5cm, fill=orange!30, minimum width=4cm },
				...
				middleLargeSpacing/.style = { xshift=1.65cm },
			}

			% server column
			\node (server) [] {\textbf{\large{Server}}};
			\node (startServer) [startfinish, below=of server, yshift=0.8cm] {Start of handshake};
			\node (serverSendChallenge) [process, below=of startServer] {Prepare client challenge; \\ Send client challenge to client};
			\node (serverVerifiesChallenge) [decision, below=of serverSendChallenge] {Is client signature valid?};
			\node (dropConnectionServer) [dropConnection, left=of serverVerifiesChallenge] {Drop\\connection};
			\node (serverSignsChallenge) [process, below=of serverVerifiesChallenge] {Sign server challenge from client; \\ Send server challenge signature and \\ public key to client};

			% invisible helper nodes
			\node (middleTop) [coord, middleLargeSpacing, right=of server] {};
			\node (middleStart) [coord, right=of serverSendChallenge] {};
			\node (below) [coord] at (middleTop |- decHashesCount) {};

			% client column
			\node (client) [right=of middleTop, middleLargeSpacing] {\textbf{\large{Client}}};
			\node (clientSignsChallenge) [process, right=of middleStart] {Sign client challenge from server; \\ Prepare server challenge; \\ Send client challenge signature, public\\ key and server challenge to server};
			\node (clientVerifiesChallenge) [decision, below=of clientSignsChallenge, yshift=-2.4cm] {Is server signature valid?};
			\node (dropConnectionClient) [dropConnection, right=of clientVerifiesChallenge] {Drop\\connection};
			\node (finishClient) [startfinish, below=of clientVerifiesChallenge] {Successful handshake};

			% arrows
			\draw [arrow] (startServer) -- (serverSendChallenge);
			\draw [arrow] (serverSendChallenge) -- (clientSignsChallenge);
			\draw [line] (clientSignsChallenge) -- (clientSignsChallenge |- serverVerifiesChallenge);
			\draw [arrow] (clientSignsChallenge |- serverVerifiesChallenge) -- (serverVerifiesChallenge);
			\draw [arrow] (serverVerifiesChallenge) -- node[anchor=north] {\scriptsize{no}} (dropConnectionServer);
			\draw [arrow] (serverVerifiesChallenge) -- node[anchor=east] {\scriptsize{yes}} (serverSignsChallenge);
			\draw [arrow] (serverSignsChallenge) -- (clientVerifiesChallenge);
			\draw [arrow] (clientVerifiesChallenge) -- node[anchor=north] {\scriptsize{no}} (dropConnectionClient);
			\draw [arrow] (clientVerifiesChallenge) -- node[anchor=east] {\scriptsize{yes}} (finishClient);
		\end{tikzpicture}
